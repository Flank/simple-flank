        ...
        androidExtension.signingConfigs {
            named("debug") {
                if (storeFile?.path?.contains("/.android/debug.keystore") == true) {
                    println("Warning: The debug keystore should be set, using the default means the cache won't work")
                }
            }
        }
    }
...
fun Project.useFixedKeystore() {
    ...
    with(copyDebugKeystore) {
        val pluginJar = zipTree(
            SimpleFlankExtension::class.java.classLoader.getResource("debugKeystore")!!.path.split("!").first()
        )
